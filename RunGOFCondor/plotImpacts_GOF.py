...
def plotImpacts_GOF(varName, year, version, makeJson=True):
	...
	print(doFits)
	mJson = sp.check_output(("combineTool.py -M Impacts -d /net/data_cms1b/user/nattland/top_analysis/{year}/{version}/output_framework/datacards/{name}/ttbar_{name}.root -m 125 -o {path}Impacts_asimov_{name}.json".format(name=varName, path=oPath, year=year, version=version).split()))
	print(mJson)
