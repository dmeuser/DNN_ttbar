...
if __name__ == "__main__":
    ...
    if mode==2:
        ...
        #  ~trainKeras(year,pathNameDict,inputVars[:nInputs],"Inlusive_noMetCut_amcatnlo_xyComponent_JetLepXY_50EP","diff_xy",targets,correctedValues,np.exp(lr),dout,np.exp(lamb),int(np.round(np.exp(batch))),int(np.round(nLayer)),nodeFac,alph,nInputs,updateInput=True,genMETweighted=False,plotOutputs=True,modelNr=modelNr,plotPurity=False)
        trainKeras(year,pathNameDict,inputVars[:nInputs],"Inlusive_noMetCut_genMETweight500enlarge_amcatnlo_xyComponent_JetLepXY_50EP","diff_xy",targets,correctedValues,np.exp(lr),dout,np.exp(lamb),int(np.round(np.exp(batch))),int(np.round(nLayer)),nodeFac,alph,nInputs,updateInput=True,genMETweighted=True,plotOutputs=True,modelNr=modelNr,plotPurity=False)
