...
def bJetRegression_Model(lr=0.0001, dout=0.3, lamb=0.05, nLayer=6, nodeFac=1., alph=0.2):
    ...
    model.add(Dense(56, input_dim=56, kernel_regularizer=regularizers.l2(regLamb)))
...
def getInputArray_allBins_nomDistr(path,inputVars,targetName,target,update,treeName,normalize=False,standardize=False,genMETweighted=False):
    ...
    onehotArr_temp1 = onehot_encoder.fit_transform(inputFeatures["Lep1_flavor"].values.reshape(-1,1))
    onehotArr_temp2 = onehot_encoder.fit_transform(inputFeatures["Lep2_flavor"].values.reshape(-1,1))
    
    inputFeatures["is_ee"] = onehotArr_temp1[:,0]*(onehotArr_temp2[:,0])
    inputFeatures["is_emu"] = onehotArr_temp1[:,0]*(onehotArr_temp2[:,1])
    inputFeatures["is_mue"] = onehotArr_temp1[:,1]*(onehotArr_temp2[:,0])
    inputFeatures["is_mumu"] = onehotArr_temp1[:,1]*(onehotArr_temp2[:,1])
    
    #  ~inputFeature.drop(columns=["Lep1_flavor", "Lep2_flavor"])
    for featName in ["is_ee","is_emu","is_mue","is_mumu"]:
        inputVars.insert(9,featName)
...
if __name__ == "__main__":
    ...
    plot_Output(dataPath,inputVars,"trainedModel_Keras/2018/2D/Inlusive_amcatnlo_xyComponent_JetLepXY_50EP__diff_xy_2018_20211126-0958genMETweighted","TTbar_amcatnlo","diff_xy",["PuppiMET*cos(PuppiMET_phi)-genMET*cos(genMET_phi)","PuppiMET*sin(PuppiMET_phi)-genMET*sin(genMET_phi)"],updateInput=True,genMETweighted=True)
