...
from tensorflow.keras.constraints import NonNeg, max_norm
...
def trainKeras(dataPath,inputVars,name,treeName,targetName,target, lr, dout, lamb, batch, nLayer, nodeFac, alph,updateInput=False,permuationImportance=False,normalize=False,standardize=False,genMETweighted=False,overSample=False,underSample=False,doSmogn=False):
...
def fit_opt(lr, dout, lamb, batch, nLayer, nodeFac, alph):
    ...
    val_MET_mean = trainKeras(dataPath,inputVars,"Inlusive_amcatnlo_xyComponent_JetLepXY_50EP","TTbar_amcatnlo","diff_xy",["PuppiMET*cos(PuppiMET_phi)-genMET*cos(genMET_phi)","PuppiMET*sin(PuppiMET_phi)-genMET*sin(genMET_phi)"], np.exp(lr), dout, np.exp(lamb), int(np.round(np.exp(batch))), int(np.round(nLayer)), nodeFac2, alph,updateInput=False,genMETweighted=False,standardize=False,overSample=False,underSample=True,doSmogn=False)
    with open("HyperParameterResuls/Fluct_logs/Fluct_logc_underSamp.csv", "ab") as f:
...
def hyperOpt():
    ...
    #  ~logger = JSONLogger(path="HyperParameterResuls/HParaLogs/logBayes_fluct_underSamp.json", reset=False)
